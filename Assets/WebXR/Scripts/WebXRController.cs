...
namespace WebXR
{
    ...
    public class WebXRController : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            {
                _t.localPosition = applyArmModel(headPosition, handRotation, headRotation);
                _t.localRotation = handRotation;
            }
            else
            {
                _t.localPosition = handPosition;
                _t.localRotation = handRotation;
            }
        ...
        public void CheckHeadandHands()
        {
            InputTracking.GetNodeStates(nodeStates);

            var headState = nodeStates.FirstOrDefault(node => node.nodeType == XRNode.Head);

            var handState = nodeStates.FirstOrDefault(node => node.nodeType == handNode);
            ...
            {
                handState.TryGetPosition(out handPosition);
                handState.TryGetRotation(out handRotation);
            }
        }
    }
}
