...
public class DebugFile : MonoBehaviour
{
    ...
    public string headerStart = "<!DOCTYPE doctype html><html><head><meta charset='utf-8'><meta content='initial-scale=1, maximum-scale=1, user-scalable=no, width=device-width, shrink-to-fit=no' name='viewport'><title>Prueba Panolens</title><script src='js/three.min.js'></script><script src='js/panolens.min.js '></script><style>html,body {width: 100%;height: 100%;overflow: hidden;margin: 0;}#container {width: 100%;height: 100%;}</style></meta></meta></head><body>";
    ...
    public string footer = "</script></body></html>";
            ...
            case tipoBoton.Video:
                {
                    ...
                    boton = boton + " hideBtnNextAndPrev(); stopVideo();   video" + idVideo + ".currentTime=0;   });  " + grupoPanel + ".add(" + NombreBoton + ");";
                }
                break;
    ...
    {
        return "<video crossorigin='anonymous' id='" + id + "' loop='' playsinline='' style='display:none'><source src='" + url + "' type='video/mp4'></source></video>";
    }
    ...
    public string CreatePanorama(string fileName, tipoPanorama pan)
    {
        ...
        {
            return "panorama = new PANOLENS.VideoPanorama('" + fileName + "', {autoplay: true, loop: true, muted: false});";
        }
        else
        {
    ...
    public string CreateInfoSpot(string infoSpotNumber, string infoSpotName, string infoSpotPosition, string InfoSGroup)
    {
        return " var " + infoSpotName + " = new PANOLENS.Infospot(175, 'assets/hsb" + infoSpotNumber + ".png" + "');" + infoSpotName + ".name = '" + infoSpotName + "';" + infoSpotName + ".position.set(" + infoSpotPosition + ");" + infoSpotName + ".addEventListener('click', function() {" + InfoSGroup + ".visible = true;});panorama.add(" + infoSpotName + ");";
    }
